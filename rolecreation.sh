...
json_arn_rp=$(aws iam create-policy --policy-name ReadOnlyS3Policy --policy-document file://Policies/s3bucket_read_only.json)

extracted_arn=$(echo "$json_arn_rp" | jq -r '.Policy.Arn')

aws iam create-role --role-name NUser --assume-role-policy-document file://Policies/trustpolicy.json
...
json_arn_ap=$(aws iam create-policy --policy-name AdminPolicy --policy-document file://Policies/PermissionPolicy.json)

extracted_arn_ap=$(echo "$json_arn_ap" | jq -r '.Policy.Arn')

aws iam create-role --role-name Admin --assume-role-policy-document file://Policies/trustpolicy.json
